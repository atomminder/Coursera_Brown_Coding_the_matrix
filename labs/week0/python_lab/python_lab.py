...
three_digits_set = {x * (base ** 2) + y * (base ** 1) + z for x in digits for y in digits for z in digits}
## Task 10
S = {1, 2, 3, 4}
T = {3, 4, 5, 6}
...
## Task 15
exclude_zero_list = [ (x,y,z) for x in S for y in S for z in S if x + y + z == 0 and  (x != y or y != z)] 

## Task 16
first_of_tuples_list =[ (x,y,z) for x in S for y in S for z in S if x + y + z == 0 and  (x != y or y != z)] [0] 
...
base = 10
digits = set(range(10))
representation_dict = {(x * (base ** 2) + y * (base ** 1) + z * (base ** 0)):[x,y,z]
                    for x in digits for y in digits for z in digits}

## Task 26
d = {0:1000.0, 1:1200.50, 2:990}
names = ['Larry', 'Curly', 'Moe']
listdict2dict = { names[index]:d[index] for index in range(len(names)) }
