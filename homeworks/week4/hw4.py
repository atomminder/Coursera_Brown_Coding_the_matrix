...
#print(rep2vec(Vec({0,1,2}, {0:2, 1:4, 2:6}), [a0,a1,a2]) == Vec({'a', 'c', 'b', 'd'},{'a': 2, 'c': 6, 'b': 4, 'd': 0}))
...
#a1 = Vec({'a','b','c','d'}, {'b':1})
#a2 = Vec({'a','b','c','d'}, {'c':1})
#print(vec2rep([a0,a1,a2], Vec({'a','b','c','d'}, {'a':3, 'c':-2})) == Vec({0, 1, 2},{0: 3.0, 1: 0.0, 2: -2.0}))
...
def is_independent(L):
    ...
    for i in range(len(L)-1):
        ...
        for i in range(len(L)):
            L_copy.append(L[i]) 
        if is_superfluous(L_copy,i) == True:
            return False
    return True
    
#vlist = [Vec({0, 1, 2},{0: 1, 1: 0, 2: 0}), Vec({0, 1, 2},{0: 0, 1: 1, 2: 0}), Vec({0, 1, 2},{0: 0, 1: 0, 2: 1}), Vec({0, 1, 2},{0: 1, 1: 1, 2: 1}), Vec({0, 1, 2},{0: 0, 1: 1, 2: 1}), Vec({0, 1, 2},{0: 1, 1: 1, 2: 0})]
#print(is_independent(vlist))
##False
#print(is_independent(vlist[:3]))
##True
#print(is_independent(vlist[:2]))
##True
#print(is_independent(vlist[1:4]))
##True
#print(is_independent(vlist[2:5]))
##True
#print(is_independent(vlist[2:6]))
##True
#print(is_independent(vlist[1:3]))
##True
#print(is_independent(vlist[5:]))
##True
...
def superset_basis(S, L):
    ...
    Example:
        ...
        True
    '''
    # ideas from https://class.coursera.org/matrix-001/forum/thread?thread_id=2945
    # avoid shallow copy
    T = S
    # use algorithm
    for i in range(len(L)):
        T.append(L[i])
        if is_independent(T) != True:
            T.pop()
    return T
...
def exchange(S, A, z):
    ...
    Example:
        ...
        True
    '''
    # refer to slides_The_Basis_11_Exchange_Lemma.pdf:page 13
    result_vec = solve(coldict2mat(S),z)
    #print(result_vec)
    for i in result_vec.f.keys():
        #print(i)
        if S[i] not in A and result_vec[i] != 0:
            return S[i]
#test
#S = [list2vec(v) for v in [[0,0,5,3],[2,0,1,3],[0,0,1,0],[1,2,3,4]]]
#A = [list2vec(v) for v in [[0,0,5,3],[2,0,1,3]]]
