...
def lin_comb_mat_vec_mult(M, v):
    ...
    # slides_The_Matrix_02_Matrix_vector_and_vector_matrix_mult:page 2
    for i in M.D[1]:
        col_dict[i] = v[i] * col_dict[i]
    for i in M.D[1]:
        result_vec += col_dict[i] 
    return result_vec

# test
# M = Mat(({1, 2, 3}, {1, 2}), {(1, 2): 0, (1, 1): 2, (2, 1): 3, (2, 2): 3})
# v = v = Vec({1,2}, {1: 2, 2:3})
...
def lin_comb_vec_mat_mult(v, M):
    ...
    # slides_The_Matrix_02_Matrix_vector_and_vector_matrix_mult:page 9
    for i in M.D[0]:
        row_dict[i] = v[i] * row_dict[i]
    for i in M.D[0]:
        result_vec += row_dict[i] 
    return result_vec


# test
#M = Mat(({1, 2}, {1, 2, 3}), {(1, 2): 2, (1, 1): 1, (2, 1): 10, (2, 2): 20,(1, 3): 3, (2, 3): 30})
#print(M)
#v = v = Vec({1,2}, {1: 3, 2:4})
#print(v)
...
def Mv_mat_mat_mult(A, B):
    ...
    # slides_The_Matrix_08_Matrix_matrix_mult: page 6
    col_dict_B = mat2coldict(B)
    for key in col_dict_B:
        col_dict_B[key] = A * col_dict_B[key]
    result_mat = coldict2mat(col_dict_B)
    return result_mat
...
def vM_mat_mat_mult(A, B):
    assert A.D[1] == B.D[0]
    result_mat = Mat((A.D[0],B.D[1]), {})
    # slides_The_Matrix_08_Matrix_matrix_mult: page 3
    row_dict_A = mat2rowdict(A)
    for key in row_dict_A:
        row_dict_A[key] = row_dict_A[key] * B
    result_mat = rowdict2mat(row_dict_A)
    return result_mat
...
def dot_prod_mat_mat_mult(A, B):
    ...
    col_dict_B = mat2coldict(B)
    result_mat = Mat((A.D[0],B.D[1]), {})
    # slides_The_Matrix_08_Matrix_matrix_mult: page 7   
    for i in result_mat.D[0]:
        for j in result_mat.D[1]: 
            result_mat[(i,j)] = row_dict_A[i] * col_dict_B[j]
    return result_mat
                       

A = Mat(({1, 2}, {1, 2}), {(1, 2): 2, (1, 1): 1, (2, 1): -1, (2, 2): 1})
print(A)
B= Mat(({1, 2}, {1, 2, 3}), {(1, 2): 2, (1, 1): 4, (2, 1): 3, (2, 2): 1,(1, 3): 0, (2, 3): -1})
print(B)
